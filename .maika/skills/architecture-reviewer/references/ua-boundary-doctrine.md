...
> Tài liệu tham khảo cho `architecture-reviewer`. Đọc trước khi kết luận về boundary, topology, async, Kafka, gRPC, hoặc cross-service.
...
|---|---|---|
| Module sở hữu domain gì? | `{{ tools.domain_relationships }}` | `{{ tools.get_dependencies }}` kiểm caller nội-service |
