...
- **BP-A6 — Guidance MINOR rơi khi migrate skill Phase 2** ✅ DONE (2026-07-05) — 3 gap MODERATE khôi phục ở `a569bbd`; 7 mục MINOR khôi phục verbatim vào references (body vẫn ≤300, lint 26/26, full 572/1): (1) `requirement-analyst` — grep = fallback tier-3 + rule "không tự ý bịa requirement"; (2) `architecture-reviewer` — doctrine "KHÔNG hardcode pattern (CQRS/MVC/Hexagonal)" + tách Fact/Nhận định/Giả định; (3) `knowledge-curator` — regenerate rule-projector sau REDIRECT→author-dna + xử lý TOKEN_LOG-missing lúc archive; (4) `openspec-explore` — "Ending Discovery" ("Handling Entry Points" hoá ra đã giữ trong examples.md, chỉ bỏ ASCII — không cần khôi phục). [BP-05, BP-03]
