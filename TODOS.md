...
- **BP-A2 — Script hóa việc deterministic trong 7 skill + 8 workflow** ⬜ TODO — targeted slice đã đóng trong PR này: `knowledge-curator` reset/archive gate dùng `gate-check reset-ready`, `spec-validator` dùng `gate-check ac-coverage` + `integration-coverage`, và 5 skill backlog đã migrate khỏi body allowlist. Phần còn lại vẫn backlog: các bước CHECK đầu approve-*/scan và broad 7-skill/8-workflow A-2, không claim full closure. Why: F-01 gap #4, F-02, bài học Pha 3 driver. Effort còn lại: làm theo cụm sau. [BP-07]
...
- **BP-A6 — Guidance MINOR rơi khi migrate skill Phase 2** ⬜ TODO — deep-read (Claude 2026-07-05) thấy kiểu rewrite thin-entrypoint làm rơi vài guidance không-mechanical; 3 gap MODERATE đã khôi phục (commit `a569bbd`), còn MINOR: (1) `requirement-analyst` — grep = fallback tier-3 + rule "không tự ý bịa requirement"; (2) `architecture-reviewer` — doctrine "KHÔNG hardcode pattern (CQRS/MVC/Hexagonal)" bị nén còn 1 dòng + tách Fact/Nhận định/Giả định; (3) `knowledge-curator` — nhắc regenerate rule-projector sau REDIRECT→author-dna (đã có ở author-dna-builder + pre-commit backstop) + xử lý TOKEN_LOG-missing lúc archive; (4) `openspec-explore` — "Ending Discovery" + "Handling Entry Points". Why: review-quality (rewrite migration lên nhánh không qua per-task review). Effort: CC ~20′, khôi phục verbatim vào references, giữ body ≤300. [BP-05, BP-03]
