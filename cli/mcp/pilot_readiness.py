...
def assess_pilot_readiness(registry: dict, provider_statuses: dict) -> dict:
    ...
    cbm = ((providers.get("codebase-memory-mcp") or {}).get("tool_contract") or {})
    exploration = set((((cbm.get("lanes") or {}).get("discovery") or {}).get("tools") or []))
    forbidden_cbm = {"index_repository", "manage_adr", "ingest_traces", "delete_project"}
    if exploration & forbidden_cbm:
        blockers.append("CBM discovery lane exposes mutation/destruction")
    ...
    for provider_id in ("understand-anything", "codebase-memory-mcp", "agent-memory", "db-access"):
    ...
    return {
        ...
        "enabled_scope": [
            "current-source inspection", "UA/CBM read-only discovery",
            "AgentMemory candidate-only recall", "DB schema/read via read-only principal",
        ],
        "disabled_scope": [
            "unscoped source writes", "CBM mutation/deletion",
