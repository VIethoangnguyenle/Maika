...
def test_recurrence_threshold_requires_three_occurrences_two_changes():
    assert candidate_triggered([
        {"recurrence_key": "x", "change_id": "A", "verified": True, "reproducible": True},
        {"recurrence_key": "x", "change_id": "A", "verified": True, "impact": "measurable"},
        {"recurrence_key": "x", "change_id": "B", "verified": True, "evaluation_ready": True},
    ])
...
def test_explicit_safe_triggers_and_unverified_feedback_rules():
    ...
    assert not candidate_triggered([{"dogfood_failure": True, "reproducible": True, "verified": True}])
