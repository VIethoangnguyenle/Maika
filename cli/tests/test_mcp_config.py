...
def test_load_json_mcp_servers(tmp_path):
    ...
    path.write_text(
        json.dumps({"mcpServers": {"db-access": {"command": "npx", "args": ["x"]}}}),
        encoding="utf-8",
    )
    ...
    assert server_names(config) == ["db-access"]
...
def test_selected_server_matches_returns_present_and_missing(tmp_path):
    ...
    present, missing = selected_server_matches(config, ["db-access", "confluence"])

    assert present == ["db-access"]
    assert missing == ["confluence"]
